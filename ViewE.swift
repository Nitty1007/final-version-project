...
struct ViewE: View {
    ...
    private let curriences = ["USD", "EUR", "GBP"]
    
    func convert (_ convert: String) -> String {
        var conversion: Double = 1.0
        let amount = Double(convert.doubhleValue)
        let selectedCurrency = curriences[itemSelected]
        let to = curriences[itemSelected2]
            ...
            Form{
                Section(header: Text("Convert a currency")) {
                    ...
                    Picker(selection: $itemSelected, label: Text("From")){
                        ForEach(0 ..< curriences.count) {
                            index in Text(self.curriences[index]).tag(index)
                        }
                    }
                ...
                Section(header: Text("Conversion")) {
                    Text("\(0) \(curriences[itemSelected2])")
                }
            }
        }
    }
